...
def _execute_local_tests(payload: CodePayload) -> TestResult:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            return TestResult(response="FAILURE", exit_code=999, logs="Timeout: Przekroczono czas wykonania.")
        except Exception as e:
            return TestResult(response="ERROR", exit_code=500, logs=str(e))
...
"""
        ...
        try:
            ...
            return TestResult(response="FAILURE", exit_code=999, logs="Błąd: Wykryto nieskończoną pętlę (Timeout).")
        except Exception as e:
            return TestResult(response="ERROR", exit_code=500, logs=str(e))
        finally:
            subprocess.run(["docker", "stop", "-t", "0", container_name], capture_output=True)
            ...
            async with websockets.connect(WEBSOCKET_SERVER_URL) as websocket:
                ...
                async for message in websocket:
                    try:
                        ...
                        logger.info(f"Wysłano wynik dla zadania (Status: {result.response})")

                    except json.JSONDecodeError:
                        logger.error("Otrzymano niepoprawny JSON.")
                    except Exception as e:
                        logger.error(f"Błąd przetwarzania wiadomości: {e}")
                        error_res = TestResult(response="ERROR", exit_code=500, logs=f"Internal Error: {str(e)}")
